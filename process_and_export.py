...
def prepare_batch_data(df, batch_size=50):
    ...
    for i in range(0, total_rows, batch_size):
        ...
        for idx, row in batch_df.iterrows():
            ...
            try:
                start_dt = pd.to_datetime(row.get("segment_start", "2024-01-01T00:00:00"))
...
def export_results_to_csv():
    ...
    try:
        ...
        print(f"   Date range: {df['segment_start'].min()} to {df['segment_start'].max()}")
...
def main():
    ...
    print(f"   Processing all records may take 2-3 hours.")
    print(f"\n   Options:")
    print(f"   1. Process ALL {len(df_train)} records")
    print(f"   2. Process first 100 records (test run)")
    print(f"   3. Process first 500 records")
    print(f"   4. Process first 1000 records")

...
def get_agriculture_data(split="train", force_download=False):
    ...
    df_filtered = df[
        df["category"].str.contains("agriculture_alimentation", case=False, na=False)
    ]
    print(
        f"Filtered {len(df_filtered)} agriculture_alimentation records from {len(df)} total records"
    )
    return df_filtered
...
def get_category_data(category, split="train", force_download=False):
    ...
    print(
        f"Filtered {len(df_filtered)} {category} records from {len(df)} total records"
    )
    return df_filtered
